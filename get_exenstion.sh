...
# You are setting up a new switchboard for your company's phone system. 
# You need to collect everyone’s current extension numbers and the pin code that they want to use when making calls.
# Because there is no internal phone, you decide to write a bash script to request this information from each 
# of your colleagues, and write it to a csv file to process later.
...
# Note: You should ensure that the extension and pin numbers are exactly 4 digits long. 
# Check out the -N option in the read command documentation, which you can access by running the command help read

# Hint: When entering private information like access codes, 
# it is always better to prevent what the user types from appearing in the terminal.
...
# Saving the data like this will make it possible to open the data in 
# Microsoft Excel or some other spreadsheet software for processing (try it out!)
...
# Ensure access code is exactly 4 digits and input is hidden
read -N 4 -s -p "What access code would you like to use when dialing in (must be 4 digits)?: " access
echo  # Add an empty line after the hidden input for better formatting

# Step 2: Saving to a CSV file
echo "$firstname,$lastname,$ext,$access" >> extensions.csv

echo "Data saved to extensions.csv"
